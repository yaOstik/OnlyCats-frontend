...
{/* --- МОБІЛЬНЕ НИЖНЄ МЕНЮ (Тільки для телефонів) --- */}
        <div className="md:hidden fixed bottom-0 w-full bg-white/90 backdrop-blur-md border-t border-gray-200 flex justify-around items-center p-3 z-50 pb-safe">
            ...
            {/* 3. Центральна кнопка "Додати пост" (з перевіркою на логін) */}
            <button
                onClick={() => { if(!isLoggedIn){setShowAuthModal(true); return;} setActiveTab('addCat'); }}
                className="bg-[#bf04ff] text-white p-3 rounded-full shadow-lg shadow-purple-500/30 -mt-6 border-4 border-[#f4f4f5] transition-transform active:scale-95"
            >
            ...
            {/* 5. Профіль (або Авторизація, якщо гість) */}
            <button
                onClick={() => setActiveTab(isLoggedIn ? 'profile' : 'auth')}
                className={`p-2 rounded-xl transition-colors ${activeTab === 'profile' || activeTab === 'auth' ? 'text-[#bf04ff]' : 'text-gray-400'}`}
            >

...
function getWeekRangeLabel(date) {
  ...
  weekStart.setDate(date.getDate() - dayFromMonday);
...
export default function RatingPage({ BASE_URL }) {
  ...
  useEffect(() => {
    const fetchRating = async () => {
      try {
        ...
        const formattedLeaders = (Array.isArray(data) ? data : []).map((post, index) => ({
          id: post.id,
          rank: index + 1,
        ...
        setLeaders(formattedLeaders.filter((item) => item.score > 0));
      } catch (requestError) {
  ...
  const weekRangeLabel = useMemo(() => getWeekRangeLabel(new Date()), []);
  ...
  const podiumStyles = [
    {
      ...
      bg: 'bg-gradient-to-t from-fuchsia-100 to-white',
      crown: '👑',
      medal: 'bg-fuchsia-400 text-white shadow-[0_0_15px_rgba(217,70,239,0.45)]',
      title: 'Supreme Fluff',
    ...
    },
    {
      ...
      bg: 'bg-gradient-to-t from-purple-100 to-white',
      crown: '🥈',
      medal: 'bg-purple-300 text-purple-900 shadow-md',
    ...
    },
    {
      ...
      bg: 'bg-gradient-to-t from-fuchsia-50 to-white',
      crown: '🥉',
      medal: 'bg-fuchsia-200 text-fuchsia-800 shadow-md',
      title: 'Bronze Paw',
      glow: 'shadow-lg',
    },
  ];
  ...
  const getCatStatus = (rank) => {
    if (rank <= 5) return { text: 'Hot Contender 🔥', color: 'text-fuchsia-600 bg-fuchsia-50' };
    if (rank <= 8) return { text: 'Rising Star ✨', color: 'text-[#d946ef] bg-[#fdf4ff]' };
    return { text: 'Cute Fluff 🐾', color: 'text-purple-600 bg-purple-50' };
  };

  if (loading) {
    return (
      <div className="w-full pb-12 animate-pulse flex flex-col items-center gap-6 mt-4">
        <div className="h-32 w-full bg-fuchsia-50 rounded-[32px] mb-8" />
        <div className="flex items-end justify-center gap-4 h-48 w-full">
          <div className="w-28 h-32 bg-gray-100 rounded-[24px]" />
          <div className="w-32 h-44 bg-gray-100 rounded-[24px]" />
  ...
  if (error) {
    return (
      <div className="text-center py-16 bg-white rounded-[32px] border border-red-50 shadow-sm mt-4">
        ...
        <p className="text-gray-400 text-sm mt-2 font-medium">Try refreshing the page.</p>
      </div>
    );
  }
  ...
  return (
    <div className="w-full pb-12 flex flex-col items-center">
      <div className="oc-rating-hero relative w-full bg-gradient-to-br from-[#fdf4ff] via-white to-[#fdf4ff] p-8 rounded-[32px] shadow-sm border border-fuchsia-100 mb-12 text-center overflow-hidden group">
        ...
        <div className="relative z-10">
          <span className="oc-rating-week-pill inline-block px-4 py-1 bg-white rounded-full text-[#d946ef] font-bold text-[11px] uppercase tracking-widest shadow-sm mb-3">
            Ongoing week {weekRangeLabel}
          </span>
          <h2 className="text-3xl font-black text-gray-900 mb-2 tracking-tight">Weekly Fluffy League</h2>
          <p className="text-[15px] text-gray-500 font-medium max-w-[90%] mx-auto leading-relaxed">
            Results: 10:00 Sunday. Top cats with the highest score stand on the podium.
          </p>
        </div>
      </div>
        ...
        <>
          <div className="flex items-end justify-center gap-3 sm:gap-6 w-full mb-16 mt-4 px-2">
            {displayOrder.map((leader) => (
              <div key={leader.id} className="flex flex-col items-center w-[32%] sm:w-[160px] relative group cursor-pointer">
                {leader.style.crown && (
                  <span
                    className={`text-4xl sm:text-5xl absolute ${
                      leader.rank === 1 ? '-top-14 animate-bounce' : '-top-10 group-hover:-translate-y-2 transition-transform'
                    } z-20 drop-shadow-md`}
                  >
                    {leader.style.crown}
                  </span>
                ...
                <div className={`w-full ${leader.style.height} ${leader.style.bg} rounded-[24px] border border-b-4 ${leader.style.border} flex flex-col justify-end items-center pb-4 shadow-sm`}>
                  <div className={`w-8 h-8 rounded-full ${leader.style.medal} font-black text-sm flex items-center justify-center mb-2 ring-4 ring-white`}>
                    {leader.rank}
                  </div>

                  <span className="font-black text-gray-900 text-[14px] w-full px-2 text-center line-clamp-2 leading-tight min-h-[34px]">
                    {leader.catName}
                  </span>
                  <span className="text-[10px] font-bold text-gray-500 uppercase tracking-wider mb-1.5">
                    @{leader.author.toLowerCase().replace(/\s+/g, '_')}
                  </span>

                  <div className="bg-white/75 backdrop-blur-sm px-3 py-1 rounded-xl flex items-center gap-1.5 shadow-sm">
                    <svg className="w-3.5 h-3.5 text-[#d946ef]" fill="currentColor" viewBox="0 0 24 24">
            ...
            <div className="w-full flex flex-col gap-4 px-1">
              ...
              {theRest.map((leader) => {
                ...
                return (
                  ...
                  >
                    <div className="absolute left-0 top-0 bottom-0 w-1.5 bg-[#d946ef] opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
                    ...
                    <div className="flex-1 min-w-0">
                      <h4 className="font-black text-gray-900 text-[17px] leading-tight truncate group-hover:text-[#d946ef] transition-colors">
                        {leader.catName}
                      </h4>
                      <div className="flex flex-wrap items-center gap-2 mt-1">
                        <p className="text-gray-400 text-[13px] font-medium truncate">
                          by @{leader.author.toLowerCase().replace(/\s+/g, '_')}
                        </p>
